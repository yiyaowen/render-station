...
BlurFilter::BlurFilter(D3DCore* pCore, int blurRadius, float blurGrade, int blurCount)
...
void BlurFilter::init() {
    ...
    createRootSig(pCore, "blur_filter", &rootSigDesc);
    ...
    horzPsoDesc.pRootSignature = pCore->rootSigs["blur_filter"].Get();
    bindShaderToCPSO(&horzPsoDesc, s_gaussianBlurHorz.get());
    horzPsoDesc.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
    checkHR(pCore->device->CreateComputePipelineState(&horzPsoDesc, IID_PPV_ARGS(&pCore->PSOs["blur_filter_horz"])));

    D3D12_COMPUTE_PIPELINE_STATE_DESC vertPsoDesc = {};
    vertPsoDesc.pRootSignature = pCore->rootSigs["blur_filter"].Get();
    bindShaderToCPSO(&vertPsoDesc, s_gaussianBlurVert.get());
    vertPsoDesc.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
    checkHR(pCore->device->CreateComputePipelineState(&vertPsoDesc, IID_PPV_ARGS(&pCore->PSOs["blur_filter_vert"])));
}

void BlurFilter::onResize(UINT w, UINT h) {
    texWidth = w;
    texHeight = h;
    createOffscreenTextureResources();
    createResourceDescriptors();
...
ID3D12Resource* BlurFilter::process(ID3D12Resource* flatOrigin) {
    ...
    pCore->cmdList->SetComputeRootSignature(pCore->rootSigs["blur_filter"].Get());
    ...
    for (int i = 0; i < _blurCount; ++i) {
        // Horizontal Blur
        pCore->cmdList->SetPipelineState(pCore->PSOs["blur_filter_horz"].Get());
        ...
        // Vertical Blur
        pCore->cmdList->SetPipelineState(pCore->PSOs["blur_filter_vert"].Get());
...
void BlurFilter::createOffscreenTextureResources() {
...
void BlurFilter::createTextureDescriptorHeap() {
...
void BlurFilter::createResourceDescriptors() {
