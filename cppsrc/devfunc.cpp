...
void dev_initCoreElems(D3DCore* pCore) {
    ...
    std::vector<VecAddDemoCSData> dataA(32);
    std::vector<VecAddDemoCSData> dataB(32);
    ...
    UINT64 byteSize = dataA.size() * sizeof(VecAddDemoCSData);
    VecAddDemoCSData* downloadData = nullptr;
    ComPtr<ID3DBlob> vecAddDemoCsInputACPU = nullptr, vecAddDemoCsInputBCPU = nullptr;
    ComPtr<ID3D12Resource> vecAddDemoCsInputAGPU = nullptr, vecAddDemoCsInputBGPU = nullptr, vecAddDemoCsOutputGPU = nullptr;
    ComPtr<ID3D12Resource> vecAddDemoCsInputAUploader = nullptr, vecAddDemoCsInputBUploader = nullptr, vecAddDemoCsOutputDownloader = nullptr;

    createDefaultBuffs(pCore, dataA.data(), byteSize,
        &vecAddDemoCsInputACPU, &vecAddDemoCsInputAGPU, &vecAddDemoCsInputAUploader);
    createDefaultBuffs(pCore, dataB.data(), byteSize,
        &vecAddDemoCsInputBCPU, &vecAddDemoCsInputBGPU, &vecAddDemoCsInputBUploader);
    checkHR(pCore->device->CreateCommittedResource(
        ...
        nullptr,
        IID_PPV_ARGS(&vecAddDemoCsOutputGPU)));
    checkHR(pCore->device->CreateCommittedResource(
        ...
        nullptr,
        IID_PPV_ARGS(&vecAddDemoCsOutputDownloader)));
    checkHR(vecAddDemoCsOutputDownloader->Map(0, nullptr, reinterpret_cast<void**>(&downloadData)));

    // Dispatch threads.
    checkHR(pCore->cmdAlloc->Reset());
    checkHR(pCore->cmdList->Reset(pCore->cmdAlloc.Get(), pCore->PSOs["simple_vec_add_demo"].Get()));

    pCore->cmdList->SetComputeRootSignature(pCore->rootSigs["simple_vec_add_demo"].Get());

    pCore->cmdList->SetComputeRootShaderResourceView(0, vecAddDemoCsInputAGPU->GetGPUVirtualAddress());
    pCore->cmdList->SetComputeRootShaderResourceView(1, vecAddDemoCsInputBGPU->GetGPUVirtualAddress());
    pCore->cmdList->SetComputeRootUnorderedAccessView(2, vecAddDemoCsOutputGPU->GetGPUVirtualAddress());
    ...
    pCore->cmdList->ResourceBarrier(1,
        &CD3DX12_RESOURCE_BARRIER::Transition(
            vecAddDemoCsOutputGPU.Get(),
            D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
            D3D12_RESOURCE_STATE_COPY_SOURCE));
    pCore->cmdList->CopyResource(vecAddDemoCsOutputDownloader.Get(), vecAddDemoCsOutputGPU.Get());
    pCore->cmdList->ResourceBarrier(1,
        &CD3DX12_RESOURCE_BARRIER::Transition(
            vecAddDemoCsOutputGPU.Get(),
            D3D12_RESOURCE_STATE_COPY_SOURCE,
    ...
    for (int i = 0; i < 32; ++i) {
        fout << "(" << downloadData[i].v1.x << ", " << downloadData[i].v1.y << ")"
            << ", (" << downloadData[i].v2.x << ", " << downloadData[i].v2.y << ", " << downloadData[i].v2.z << ")" << std::endl;
    }
    fout.close();

    vecAddDemoCsOutputDownloader->Unmap(0, nullptr);
}
